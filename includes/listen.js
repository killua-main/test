module.exports = function ({ api, models }) {
  setInterval(function () {
            ...
            await new Promise(async resolve => {
                for (const checkttFile of checkttData) {
                    ...
                    checkttBody += storage.slice(0, 10).map(item => {
                        return ` Top ${count++} : ${item.name} tổng ${item.count} tin nhắn`;
                    }).join('\n');
    ...
    logger(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "Bot Cua Vtoan" : global.config.BOTNAME}`, "[ Thông tin Bot ]");
  ...
  const handleRefresh = require("./handle/handleRefresh")({  api, Threads, Users, Currencies, models });
    ...
    return async (event) => {
        if (event.type == "change_thread_image") api.sendMessage(`» [ Update New ]\n»  ${event.snippet}`, event.threadID);
        switch (event.type) {
            case "message":
            case "message_reply":
            case "message_unsend":
